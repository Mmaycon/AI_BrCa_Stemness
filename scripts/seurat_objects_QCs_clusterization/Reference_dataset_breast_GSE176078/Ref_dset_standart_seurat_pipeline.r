...
saveRDS(Sobj, file = "/mnt/plummergrp/maycon/Reference_dataset_standart_seut_pipeline.rds") # there is a newest version of this object called "Reference_dataset_standart_seut_pipeline_STEMNESScomputed.rds" saved in "/mnt/plummergrp/maycon/Figures_to_paper/Ref_dset/Reference_dataset_standart_seut_pipeline_STEMNESScomputed.rds" and made in "/mnt/plummergrp/maycon/Figures_to_paper/stemness_index_prediction/2023_03_09_Stemness_index/stemness_index_v2.r"
...
### Adding Stemness predicted only on Tumors (Ref and Validation datasets) and iPS (no Normal samples were included here) ### 

# Seurat object reference
Sobj_Ref = readRDS("/mnt/plummergrp/maycon/Figures_to_paper/Ref_dset/Reference_dataset_standart_seut_pipeline_STEMNESScomputed.rds")
bk = Sobj_Ref
...
stemnes_index$stemness
head(stemnes_index)

length(intersect(Sobj_Ref@meta.data$cell_barcode, 
                 rownames(stemnes_index))) #80682

stemnes_index$cell_barcode = rownames(stemnes_index)
names(stemnes_index)[names(stemnes_index) == "stemness"] <- "noNormal_stemness"
names(Sobj_Ref@meta.data)[names(Sobj_Ref@meta.data) == "stemness"] <- "all_stemness"

# Merge, cbind, and over writte (merge option 1)
meta_data = Sobj_Ref@meta.data
...
Sobj_Ref@meta.data = meta_data_to_add_indeed
